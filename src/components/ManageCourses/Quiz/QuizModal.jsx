...
import { Button, Dialog, DialogActions, DialogContent, DialogTitle, TextField, Radio, FormControlLabel, RadioGroup, FormControl } from '@material-ui/core';
...
function QuizModal({ isOpen, onClose, onSave, quizDetail }) {
    ...
    useEffect(() => {
        ...
        if (token && quizDetail) {
            fetchData(`/result-detail/by-result-quiz?result_quiz_id=${quizDetail.id}`, token).then((resp) => {
                if (resp) {
