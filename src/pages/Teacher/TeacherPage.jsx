...
import { CourseControllerApi, FeedbackControllerApi, PaymentHistoryControllerApi, TransactionControllerApi, WalletControllerApi, WithdrawRequestControllerApi } from '../../api/generated/generate-api';
import { calculatePercentageChange, filterObjectsByCurrentMonth, filterObjectsByCurrentWeek, filterObjectsByLastMonth, formatVND, getChartData, getChartRevenues, getRevenueBigChartData } from '../../util/Utilities';
...
const TeacherDashboard = () => {
    ...
    const fetchFeedbackData = async (res) => {
        let data = [
            { name: "5 sao", value: 0, color: "#0088FE" },
            { name: "4 sao", value: 0, color: "#00C49F" },
            { name: "3 sao", value: 0, color: "#FFBB28" },
            { name: "2 sao", value: 0, color: "#FF8042" },
            { name: "1 sao", value: 0, color: "#2d3436" },
        ];
        let feedbackDetails = [];
        for (const r of res) {
            try {
    ...
    useEffect(() => {
        transactionApi.getTransactionForWithdraw(

            parseInt(userTmp?.teacherId)
            ,
            (err, res) => {
                if (res) {
                        ...
                        <div className={classNames(Styles.contentContainer)}>
                            <div className={classNames(Styles.home)}>
                                <div className={classNames(Styles.box, Styles.box1)}>
                                    <TopBox title={'Nạp/rút'} transactions={transactions} />
                                </div>
